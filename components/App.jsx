...
// ── Constants ────────────────────────────────────────────────────
const STORAGE_KEY = "mandarinApp_v3";
...
// ── AI Generation ────────────────────────────────────────────────
async function generateWords(settings, context = "", lastWeekWords = [], excludeIds = []) {
  ...
  const lastWeekNote = lastWeekWords.length > 0
    ? `Last week's words were: ${lastWeekWords.map(w => `${w.simplified} (${w.english})`).join(", ")}. Naturally recycle any that may not have had time to stick, but don't repeat words that are clearly already part of the family's vocabulary.`
    : "";

  const excludeNote = excludeIds.length > 0
    ? `Do NOT include these words (already used or forfeited this week): ${excludeIds.join(", ")}.`
    : "";

  const splitNote = hasContext
    ? `Generate ${totalWords} words: EXACTLY 2 related to this week's context ("${context}") and ${totalWords - 2} evergreen words for daily family life. Do not let the context dominate.`
...
- Each word needs a very specific usage tip (exactly WHEN/HOW to say it during the day)
...
async function sendConfirmationEmail(settings, week) {
  ...
  `);
  if (settings.userEmail) await sendEmail(settings.userEmail, `🈶 This week's Mandarin — ${week.theme}`, html);
  if (settings.partnerEmail) await sendEmail(settings.partnerEmail, `🈶 This week's Mandarin — ${week.theme}`, html);
}
...
async function sendMidweekEmail(settings, week) {
  ...
  const html = emailShell(`
    ...
    <a href="${APP_URL}" style="display:block;text-align:center;background:#1A1A2E;color:#fff;text-decoration:none;padding:14px;border-radius:12px;font-weight:600;font-size:15px;margin-top:24px;">Open app →</a>
  `);
  if (settings.userEmail) await sendEmail(settings.userEmail, `今日 · Midweek Mandarin reminder`, html);
  if (settings.partnerEmail) await sendEmail(settings.partnerEmail, `今日 · Midweek Mandarin reminder`, html);
}

async function sendSaturdayPromptEmail(settings) {
  const html = emailShell(`
    <h1 style="font-size:26px;font-weight:700;color:#1A1A2E;margin:0 0 12px;">New week, new words 🈶</h1>
    <p style="font-size:15px;color:#1A1A2E;line-height:1.6;margin:0 0 24px;">It's Saturday — time to set this week's Mandarin words. Open the app, add any context for the week, and confirm your words.</p>
    <a href="${APP_URL}" style="display:block;text-align:center;background:#1A1A2E;color:#fff;text-decoration:none;padding:14px;border-radius:12px;font-weight:600;font-size:15px;">Get this week's words →</a>
  `);
  if (settings.userEmail) await sendEmail(settings.userEmail, `🈶 Time for this week's Mandarin words`, html);
  if (settings.partnerEmail) await sendEmail(settings.partnerEmail, `🈶 Time for this week's Mandarin words`, html);
}

async function sendShareEmail(toEmail, fromName, settings) {
  const html = emailShell(`
    ...
    <p style="font-size:15px;color:#1A1A2E;line-height:1.6;margin:0 0 16px;">Each week it generates 4 Mandarin words themed around what's actually happening in your family — your kids' ages, milestones, sibling dynamics. Each word comes with a specific tip for when to say it during the day, so it becomes a habit rather than homework.</p>
  ...
  return (
    <div style={{ background: C.bg, minHeight: "100vh", maxWidth: 430, margin: "0 auto", fontFamily: "Inter, sans-serif", color: C.navy, paddingBottom: 80 }}>
      ...
      <div style={{ padding: "24px 20px" }}>
        {tab === "week" && (
          selectedWord
            ? <WordDetail word={selectedWord} onClose={() => setSelectedWord(null)} appState={appState} onUpdateState={updateState} />
            : <WeekScreen appState={appState} onUpdateState={updateState} onSelectWord={setSelectedWord} />
        )}
        {tab === "history" && <HistoryScreen appState={appState} />}
        {tab === "settings" && <SettingsScreen appState={appState} onUpdateState={updateState} />}
      </div>

      <BottomNav tab={tab} setTab={(t) => { setTab(t); setSelectedWord(null); }} />
    </div>
  );
}

// ── Week Screen ──────────────────────────────────────────────────
function WeekScreen({ appState, onUpdateState, onSelectWord }) {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      const result = await generateWords(appState.settings, context, lastWeekWords);
  ...
  async function handleSwap(wordId) {
    ...
    try {
      const excludeIds = appState.currentWeek.words.map(w => w.simplified);
      const lastWeekWords = appState.history?.[0]?.words || [];
      ...
      const newWord = { ...result.words[0], status: "active" };
      const newWords = appState.currentWeek.words.map(w => w.id === wordId ? { ...newWord, id: wordId } : w);
  ...
  async function handleConfirm() {
    ...
    await sendConfirmationEmail(appState.settings, appState.currentWeek);
    setConfirming(false);
  }
    ...
    return (
      <div style={{ textAlign: "center", paddingTop: 40 }}>
        ...
        <div style={{ fontSize: 15, color: C.muted, lineHeight: 1.6, marginBottom: 32 }}>
          Ready for this week's words?
        </div>
        <button onClick={() => setStep("context")} style={btnStyle(C.navy)}>
    ...
    return (
      <div>
        ...
        <div style={{ fontSize: 14, color: C.muted, marginBottom: 20, lineHeight: 1.6 }}>
          A holiday, a milestone, something Austin is into. Optional — leave blank for everyday words.
        </div>
        <textarea
          value={context}
        ...
        />
        {error && <div style={{ color: "#E07A5F", fontSize: 13, marginTop: 8 }}>{error}</div>}
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        {confirmed && <div style={{ fontSize: 12, color: C.amber, fontWeight: 600, marginTop: 6 }}>✓ Confirmed — email sent</div>}
      </div>

      {/* Words */}
      {currentWeek.words.map(word => (
        <div key={word.id} style={{ background: C.card, borderRadius: 14, padding: "16px 18px", marginBottom: 10, borderLeft: `3px solid ${C.accent}`, cursor: "pointer" }}
          onClick={() => !swappingId && onSelectWord(word)}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div>
              <div style={{ fontFamily: "Noto Serif TC, serif", fontSize: 38, fontWeight: 700, lineHeight: 1.1, marginBottom: 4 }}>
                {word.simplified}
                {word.traditional !== word.simplified && <span style={{ fontSize: 26, color: C.muted, marginLeft: 8 }}>{word.traditional}</span>}
              </div>
            ...
            {!confirmed && (
              <button
                onClick={e => { e.stopPropagation(); handleSwap(word.id); }}
                disabled={!!swappingId}
                style={{ background: "none", border: `1.5px solid ${C.border}`, borderRadius: 8, padding: "6px 12px", fontSize: 12, color: C.muted, cursor: "pointer", whiteSpace: "nowrap", marginTop: 4 }}
              >
                {swappingId === word.id ? "…" : "Swap"}
              </button>
            )}
          </div>
        </div>
      ...
      {!confirmed && (
        <button
          ...
          disabled={confirming}
          style={{ ...btnStyle(C.navy), width: "100%", marginTop: 8, opacity: confirming ? 0.6 : 1 }}
        >
...
function DetailBlock({ label, content, isChinese }) {
  return (
    <div style={{ marginBottom: 20 }}>
      ...
      <div style={{ background: C.card, borderRadius: 12, padding: "14px 16px", fontSize: isChinese ? 17 : 15, fontFamily: isChinese ? "Noto Serif TC, serif" : "Inter, sans-serif", lineHeight: 1.6 }}>
        {content}
      </div>
    </div>
  );
...
function SettingsScreen({ appState, onUpdateState }) {
  ...
  async function handleShare() {
    ...
    await sendShareEmail(shareEmail, form.userName, appState.settings);
    setShareStatus("Sent! ✓");
    setShareEmail("");
    ...
    <div>
      ...
      <SettingsSection title="Emails">
        ...
        <div style={{ fontSize: 12, color: C.muted, padding: "8px 16px 12px", lineHeight: 1.5 }}>
          Saturday prompt · Midweek reminder (Wed 7am)
        </div>
      </SettingsSection>

      <button onClick={handleSave} style={{ ...btnStyle(saved ? "#4CAF50" : C.navy), width: "100%", marginBottom: 20 }}>
        {saved ? "Saved ✓" : "Save settings"}
      </button>

      {/* Send now — only shown after settings saved and week confirmed */}
      ...
      <SettingsSection title="Share with a friend">
        <div style={{ padding: "12px 16px" }}>
          <input
            ...
            style={{ width: "100%", background: "none", border: "none", outline: "none", fontSize: 15, color: C.navy, fontFamily: "Inter, sans-serif", marginBottom: 12 }}
          />
          <button onClick={handleShare} style={{ ...btnStyle(C.teal), width: "100%" }}>
            Send invite
          </button>
